...
```
After allocation of 160 bytes on stack frame, at line `main+49` space to store copied arguments on stack is setted to 0.
...
```
The binary contains two user defined functions and four system functions. Among the `strcat()` is know for security-sensivity.
Greeting message lays on `0xbffff5a0` and savec EIP on `0xbffff5ec`.
If global variable `language` is not setted to fi or nl, it will not be possible to overwrite whole word `0xbffff56c`
```gdb
